 ...
 * <p>
 * AutowireCapableBeanFactory 直接继承自 BeanFacotory，它扩展了自动装配的功能，根据类定义 BeanDefinition 装配 Bean、执行后置处理器等。值得注意的是，这个接口并没有被 ApplicationContext 继承，
 * 因为应用程序几乎不用用到这些接口，当然也可以通过 ApplicationContext#getAutowireCapableBeanFactory() 获得这个 BeanFactory 实例。
 * <p>
 * <p>
 * <p>
...
public interface AutowireCapableBeanFactory extends BeanFactory {
	...
	/**
	 * 把与 Bean 的属性具有相同的名字的其他 Bean 自动装配到这个属性上。举个例子就是：当有一个属性名字为 person 时，则自动装配策略选择 id 为 person 的 Bean 进行装配
	...
	/**
	 * 把与 Bean 的属性具有相同的类型的其他 Bean 自动装配到这个属性。
	...
	/**
	 * 把与 bean 的构造器入参具有相同类型的其他 Bean 自动装配到 Bean 构造器的对应参数中。
	...
	/**
	 * bean 生命周期相关的初始化，如 BeanFactoryAware#setBeanFactory、InitializingBean#afterPropertiesSet
	...
	/**
	 * 在 Spring 获取 bean 的规则中有这样一条：尽可能保证所有 bean 初始化后都会调用注册的 BeanPostProcessor 的 postProcessAfterInitialization 方法进行处理，在实际开发过程中大可以针对此特性设计自己的业务逻辑。
	 * Apply {@link BeanPostProcessor BeanPostProcessors} to the given existing bean
	...
	/**
	 * 属性注入时根据类型查找依赖，支持 List<BeanA> list 集合类型注入
